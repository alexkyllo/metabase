...
(extend OracleDriver
  ...
  (merge (sql/ISQLDriverDefaultsMixin)
         {:apply-limit               apply-limit
          ...
          :date                      date
          :excluded-schemas          (constantly #{"APPQOSSYS"
                                                   "CTXSYS"
                                                   "DBSNMP"
                                                   "GSMADMIN_INTERNAL"
                                                   "OUTLN"
...
;; (defn- query [sql]
;;   ((resolve 'metabase.test.data.oracle/query) sql))


;; (defn- a [] (query "SELECT TRUNC(SYSDATE, 'month') AS \"M\" FROM DUAL"))

;; (defn- entity []
;;   (sql/korma-entity (OracleDriver.) ((resolve 'metabase.test.data.oracle/database->connection-details)) {:name :CAM.incidents}))

;; (defn- a []
;;   (k/select (entity)
;;             (k/fields (date :week-of-year :timestamp))
;;             (k/group (date :week-of-year :timestamp))))

;; #_(kx/inc (kx/floor (kx// (kx/inc (kx/- (date :d v) (trunc :year v)))
;;                           (k/raw 7))))

;; [1 8 15 22 29 36 43 50 57 64 71 78 85 92 99 106 113 120 127 134 141 148 155 162 169 176 183 190 197 204
;;  211 218 225 232 239 246 253 260 267 274 281 288 295 302 309 316 323 330 337 344 351 358]

;; (defn- b []
;;   (let [v   (unix-timestamp->timestamp nil :CAM.incidents.timestamp :milliseconds)
;;         woy (date :minute-of-hour v)]
;;     (k/select (entity)
;;               (k/fields [(k/raw "COUNT(*)") :count] [woy :woy])
;;               (k/group woy)
;;               (k/order woy :ASC))))
