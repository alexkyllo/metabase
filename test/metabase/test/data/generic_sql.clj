...
(defn load-data-with-debug-logging
  ...
  [insert!]
  (fn [rows]
    (println (u/format-color 'blue "Inserting %d rows like:\n%s" (count rows) (k/sql-only (k/insert :some-table (k/values (first rows))))))
...
(def load-data-one-at-a-time-parallel!  "Insert rows one at a time, in parallel."              (make-load-data-fn load-data-with-debug-logging load-data-add-ids (partial load-data-one-at-a-time pmap)))
